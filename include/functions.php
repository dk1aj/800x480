...
function getTXInfo() {
	if (file_exists("/var/log/svxlink")) { $txstat = exec('egrep -a -h "transmitter|squelch is|squelch for" /var/log/svxlink | tail -1');} 
	if ((strpos($txstat, 'ON') !== false)) { 
		 ...
		 $diff=$tmst-$tmss;
		    ...
		    } else { $txs="<tr><td style=\"background:#4aa361  ;color:white;\"><div style=\"margin-top:2px;margin-bottom:2px;font-weight:bold;\">RX Audio</div></td></tr>\n";
                }
              return $txs;
	    }
	elseif ((strpos($txstat, 'OPEN') !== false)) { 		
             return "<tr><td style=\"background:#ff6600;color:black;\"><div style=\"margin-top:2px;margin-bottom:2px;font-weight:bold;\">TXing</div></td></tr>\n";
