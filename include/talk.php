<?php
// 2025-05-05 14:13:50 Sicherheitskonstante setzen, um Logikänderungen zu unterbinden
...
if ($file) {
    ...
    if ($lastCallsign !== null && $lastTGNumber !== null) {
        ...
        if (!empty($userInfo) && count($userDetails) >= 3 &&
            ...
            echo '<div style="position: absolute; top: 55%; left: 10%; font-size: 36px; color: grey; text-align: center;">';
        ...
        if (!empty($tgmon)) {
            // Button-Anzeige für überwachte TGs
            ...
            foreach ($tgmon as $tg) {
                ...
                echo "<button style=\"min-width: 30px; height: 40px; font-size: $fontSize; background-color: $bgColor; color: $color; border: none; border-radius: 12px;\">";
                echo "TG $tg";
                echo "</button>";
            }
            echo '</div>';
        ...
        if (!empty($tgselect)) {
            ...
            echo '<span style="color: blue; font-size: 48px;">last TG#' . htmlspecialchars($tgselect) . '</span>&nbsp;&nbsp;';
            ...
            // Einstellbare Fontgröße für Netzname
            $fontSizeNet = '110px'; // z. B. aus Config

            echo '<div style="color: white; font-size: 160px; line-height: 160px; margin: 0;">' . $currentTime . '</div>';
            echo '<span style="color: cyan; font-size: ' . $fontSizeNet . '; line-height: ' . $fontSizeNet . '; margin: 0; display: block;">' . htmlspecialchars($fmnet) . '</span>';
        }
